...
def create_reservation(request):
    ...
    if not (flight_data and hotel_data):
        ...
        reservation_result = create_combined_reservation(flight_data, hotel_data)
        if isinstance(reservation_result, dict) and 'error' in reservation_result:
            return Response({'error': reservation_result['error']}, status=status.HTTP_400_BAD_REQUEST)
        elif reservation_result:
            return Response({'message': 'Reservation created successfully', 'reservation_id': reservation_result.id}, status=status.HTTP_201_CREATED)
        else:
